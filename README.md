...
npm run lint          # Lint
npm run check:glyphs  # 字幕がフォント収録文字だけで書かれているか検査
...
字幕の描画には映画字幕風フォント「[しねきゃぷしょん](https://fontfree.me/235)」（chiphead 作）を使用しています。収録は 4370 グリフで、JIS 第一水準の漢字は全字収録していますが、第二水準は一部（647 字）のみです。未収録の文字はフォールバックの明朝体で描画され、1 つの字幕の中で書体が混ざってしまいます。

字幕を追加したら `npm run check:glyphs` で全字幕を検査してください（未収録の文字があれば該当箇所を表示して失敗します）。CI（`.github/workflows/ci.yml`）でも自動実行され、PR の差分に注釈が付きます。
